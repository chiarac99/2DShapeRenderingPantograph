...
const float LINK_A1 = 0.10f;    // proximal link, motor 1 side
const float LINK_A2 = 0.10f;    // distal link, motor 1 side (P2 -> P3)
const float LINK_A3 = 0.10f;    // distal link, motor 5 side (P4 -> P3)
const float LINK_A4 = 0.10f;    // proximal link, motor 5 side
...
const float R_SECTOR  = 0.048f;     // m, sector pulley radius         (A3: 0.075)
...
const float ENC_M    = -0.0221f;            // deg per encoder count (motor 1, R^2=0.997)
const float ENC_B    = 0.0f;             // deg offset; updatedPos=0 at 0deg by design
...
//
// NOTE: Each shape array currently ends with a duplicate of the first
// point (a relic of the Python script's "close the curve" step). The
// findNearestSegment loop guards against the resulting zero-length
// segment, but consider trimming the duplicate (and dropping N to 199)
// in the next Python regen for cleanliness.
...
// ============================================================
// Declared up here (above loop) so the telemetry print can reference
// updatedPos directly without needing extern forward declarations.
...
void   computeMotorTorques(float Fx, float Fy, float &tau1, float &tau5, float theta_self);
void   sendMotorCommand(float tau1, float tau5, float theta_self);
...
void setup() {
  ...
  linkSerial.setTimeout(10); // cap parseFloat blocking at 10 ms
...
void loop() {
  ...
  // Sample encoder ONCE per loop. Pass cached value to all functions that
  // need it (rather than re-reading inside each function), since re-reads
  // were confusing the flip-detector during fast motion.
  float theta_self_cached = readThisBoardTheta();

  float xh, yh;
  getPenTipPosition(xh, yh, theta_self_cached);
  ...
  // ---- SAFETY: force motors OFF for calibration test ----
  Fx = 0.0f;
  Fy = 0.0f;

  float tau1, tau5;
  computeMotorTorques(Fx, Fy, tau1, tau5, theta_self_cached);

  sendMotorCommand(tau1, tau5, theta_self_cached);

  // Telemetry: print updatedPos every 50 loops so you can watch it sweep
  static int printCounter = 0;
  if (++printCounter >= 50) {
    float ts_deg = ENC_M * updatedPos + ENC_B;
    Serial.print("updatedPos=");  Serial.print(updatedPos);
    Serial.print("  theta_deg=");  Serial.print(ts_deg, 1);
    Serial.print("  partner_rad=");  Serial.print(theta_partner_rad, 3);
    Serial.print("  xh,yh=");     Serial.print(xh, 4);
    Serial.print(",");            Serial.print(yh, 4);
    Serial.println();
    printCounter = 0;
  }
}
...
// NOTE: this is called multiple times per loop (in getPenTipPosition,
// computeMotorTorques, and sendMotorCommand's theta broadcast). The flip
// state machine assumes consecutive calls correspond to consecutive
// physical timesteps; if you observe flip-counting issues, refactor to
// sample once per loop and cache the result.
...
bool computeFwdKin(float theta1, float theta5, float &x3, float &y3) {
  ...
  if (doPrint) {
    Serial.print("  FK: t1=");Serial.print(theta1,3);
    Serial.print(" t5=");Serial.print(theta5,3);
    Serial.print(" P2=("); Serial.print(x2,3); Serial.print(",");Serial.print(y2,3);
    Serial.print(") P4=("); Serial.print(x4,3); Serial.print(",");Serial.print(y4,3);
    Serial.print(") dP2P4="); Serial.print(dP2P4,4);
    Serial.println();
  }
  
  if (dP2P4 < 1e-6f) { if(doPrint) Serial.println("  -> ret false (singular)"); return false; }

  float dP2Ph = (LINK_A2*LINK_A2 - LINK_A3*LINK_A3 + dP2P4*dP2P4) / (2.0f * dP2P4);
  float h2 = LINK_A2*LINK_A2 - dP2Ph*dP2Ph;
  
  if (doPrint) {
    Serial.print("  dP2Ph="); Serial.print(dP2Ph,4);
    Serial.print(" h2="); Serial.print(h2,6);
    Serial.println();
  }
  
  if (h2 < 0.0f) { if(doPrint) Serial.println("  -> ret false (h2 negative)"); return false; }
  ...
  if (doPrint) {
    Serial.print("  Ph=("); Serial.print(xh_pt,3); Serial.print(",");Serial.print(yh_pt,3);
    Serial.print(") P3=("); Serial.print(x3,3); Serial.print(",");Serial.print(y3,3);
...
void getPenTipPosition(float &x, float &y, float theta_self) {
  // theta_self is passed in (cached from loop()) instead of re-reading
  // the encoder, which was triggering spurious flip counts during fast motion.
...
//
// IMPORTANT: skips zero-length segments. Without this guard, a degenerate
// segment (e.g. the duplicate first/last point in the closed shape array)
// produces seg_len2 = 0, then t = something/0 = Inf/NaN. AVR's software
// FP library handles NaN arithmetic ~30-100x slower than normal floats,
// which dragged the loop from ~5 ms to ~6 seconds before we caught it.
int findNearestSegment(float x, float y, float &dist, float &nx, float &ny,
                       float &t_frac) {
  int best_i = 0;
  ...
  for (int i = 0; i < N_POINTS; i++) {
    ...
    // Skip degenerate (zero-length) segments to avoid divide-by-zero NaN.
    if (seg_len2 < 1e-12f) continue;
...
void computeMotorTorques(float Fx, float Fy, float &tau1, float &tau5, float theta_self) {
  // theta_self is passed in (cached from loop()) instead of re-reading
  // the encoder, which was triggering spurious flip counts.
#ifdef IS_MOTOR_1
  float theta1 = theta_self;
  float theta5 = theta_partner_rad;
#else
...
void sendMotorCommand(float tau1, float tau5, float theta_self) {
  ...
  if (++thetaCounter >= 20) {
    linkSerial.print("A");
    linkSerial.println(theta_self, 3);    // use cached value, no encoder re-read
    thetaCounter = 0;
  }
}
